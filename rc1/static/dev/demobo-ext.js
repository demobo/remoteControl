var rcPort = window.demoboRcPort ? window.demoboRcPort : 1280; //this is set through bookmarklet, default port of rc is 1280
var demoboPort = window.demoboPort ? window.demoboPort : 1281; //this is set through bookmarklet, default port of demobo is 1281
...
function loadMain(){
  ...
  var base;
  if (dev){
    //assume localhost:1238 is the test environment, which is set in Google AppEngine
...
function loadUtil(){
  ...
  var s = document.createElement('script');
  s.src = dev?'http://localhost:'+demoboPort+'/dev/util.js':'http://www.demobo.com/util.js?1211';
  s.setAttribute('class', 'dmb-script');
  s.onload = loadMain;
  document.body.appendChild(s);
}
...
function loadDemoboApi(){
  console.log('jquery loaded, here at loadDemoboApi');
    var s = document.createElement('script');
    s.src = dev?'http://api.demobo.com/js_demobo.js':'http://api.demobo.com/js_demobo.js';
    s.setAttribute('class','dmb-script');
    s.onload = loadUtil;
    document.body.appendChild(s);
}
